...
import group24.escaperoom.entities.properties.Stylable.Style;
...
import group24.escaperoom.ui.editorTools.ItemDrawer.ItemCategory;
import group24.escaperoom.ui.editorTools.ItemDrawer.ItemNode;
...
public class EditorUI {
  ...
  public class ItemHint extends SmallLabel {
  ...
  public class CoordHints extends HorizontalGroup {
    ...
    public CoordHints() {
      maxX = LevelEditorScreen.WORLD_WIDTH - 1;
      maxY = LevelEditorScreen.WORLD_HEIGHT - 1;
      space(5);
      addActor(coords);
    }
  ...
  public EditorUI(LevelEditorScreen screen, ToolManager tools, DragManager drag, ActionHistory history,
      ItemDrawer drawer) {
  ...
  private void buildUI(LevelEditorScreen screen) {
    root.defaults();
    ...
    sideBar.add(
      new MenuEntryBuilder(sideBar, "Game Settings")
        .spawns((p) -> {
          ...
          m.add(
            new MenuEntryBuilder(m, "Persistent FOW")
                .selectable(screen.getMetadata().gameSettings.persistentReveal)
                .onSelect(() -> {
  ...
  private void buildItemDrawer(LevelEditorScreen screen) {
    ...
    for (String category : categories) {
      ...
      ObjectTypeData[] typeDatas;
      if (usesDefaultItems) {
        typeDatas = LoadedObjects.getItems(category).toArray(new ObjectTypeData[0]);
      } else {
        typeDatas = LoadedObjects.getUserItems(category).toArray(new ObjectTypeData[0]);
      }
